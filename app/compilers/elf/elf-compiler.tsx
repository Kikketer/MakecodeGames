...
export default function ElfCompiler() {
  ...
  const compile = useCallback(
    async (file: File) => {
      ...
      try {
        const resp = await fetch("/api/compile", { method: "POST", body: form });
        ...
        const projectName = resp.headers.get("X-Project-Name") ?? "output";
        const filename = `${projectName.replace(/[^a-zA-Z0-9_-]/g, "_")}.elf`;
        const blob = await resp.blob();
        const url = URL.createObjectURL(blob);

        appendLog([`Done — ${(blob.size / 1024).toFixed(1)} KB ELF ready.`]);
